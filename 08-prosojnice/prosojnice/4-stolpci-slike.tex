		...
		\centering
		  ...
		  \begin{tikzpicture}
			%Spodnje je za nalogo 3.4.
					...
					\coordinate (A') at ({-\td},{\tt});
					\draw[gray, thin] (A') arc[start angle=60, end angle=-60, radius=2] node[left] {\scriptsize $m$};\pause

					% Naloga 3.4.3.: Definirajte točko T'' in narišite lok n z oznako.
					\coordinate (A') at ({\td},{\tt});
					\draw[gray, thin] (A') arc[start angle=120, end angle=240, radius=2] node[right] {\scriptsize $n$};\pause

					% Naloga 3.4.4.: Definirajte in narišite točko C.
					\coordinate [label=below:{\scriptsize $C$}] (C) at (0,{-\t});
					\draw (C) circle (1.5pt);\pause
